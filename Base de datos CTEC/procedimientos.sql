create function add_comment(cid integer, sid integer, pcomment character varying) returns character varying as $message$
	...
	begin 
		...
		where construction_stage.construcion_id = cid and construction_stage.stage_id = sid;
...
create function getClients() returns table(id int, _name character varying, lastName character varying, identity_number character varying, cell_phone_number character varying, username character varying) as $$
	begin
	  return query select general_user.id, general_user._name, general_user.lastName, general_user.identity_number, general_user.cell_phone_number , general_user.username
		from general_user inner join user_rol on general_user.id = user_rol.user_id
		where user_rol.rol_id = 2;
	end;
	$$ language plpgsql;
	
create function getMaterialsByStageConstruction(pstage_id int, pcons_id int)
	returns table(description character varying, quantity int) as $$
	begin 
	  return query select M.m_name, SM.quantity
	   from stage_materials as SM inner join material as M on SM.material_id = M.id
	   where SM.stage_id = pstage_id and SM.c_id = pcons_id;
	end;
	$$ language plpgsql;
...
CREATE OR REPLACE FUNCTION insert_pedido_to_stage(pstage_id integer, pcons_id integer, pwish character varying)
...
create function select_all_construction()
	returns table (id integer, c_location character varying, user_id integer) as $$
	begin 
		return query select *
			from construction;
	end;
...
create function select_all_user()
	returns table (id integer, _name character varying, lastname character varying, identity_number character varying, cell_phone_number character varying, username character varying) as $$
	begin 
...
create function select_construction_by_material(pname character varying)
	returns table (id integer, c_location character varying) as $$
	begin
...
create function select_stage_by_construction(pid int)
	returns table (id integer, description character varying, start_day character varying, finish_day character varying) as $$
	begin 
		return query select S.id, S.description, CS.start_day, CS.finish_day
...
create function select_construction_by_start_day(pday character varying) 
	returns table (id int, c_location character varying) as $$
	begin 
		return query select distinct(C).id, C.c_location
			from construction_stage as CS inner join construction C on CS.construcion_id = C.id 
			where ((select cast(split_part(pday, '-', 1) as int)) <= 16) 
			and (((select cast(split_part(pday, '-', 2) as int)) = (select cast(split_part(CS.start_day, '-', 2) as int))))
			and ( (select cast(split_part(CS.start_day, '-', 1) as int)) <= 31) and (16 <= (select cast(split_part(CS.start_day, '-', 1) as int)));
	end;
	$$ language plpgsql;
